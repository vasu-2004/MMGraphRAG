...
$$
\mathrm { C E } = - \frac { 1 } { N } \sum _ { i } \sum _ { j \in \{ 0 , 1 \} } y _ { i j } \log p _ { i j }
$$
...
$$
\mathbf { C E } = - \frac { 1 } { N } \sum _ { i } \alpha _ { i } \sum _ { j \in \{ 0 , 1 \} } y _ { i j } \log p _ { i j }
$$
...
$$
\mathrm { D S C } ( A , B ) = \frac { 2 | A \cap B | } { | A | + | B | }
$$
...
<table><tr><td></td><td>LossFormula (one sample xi)</td></tr><tr><td>CE</td><td>-∑je{0,1) Yij log Pij</td></tr><tr><td>WCE</td><td>-ai ∑j∈{0,1} Yij log Pij</td></tr><tr><td>DL</td><td>2piiyii+Y p²+y²+γ</td></tr><tr><td>TL</td><td>1 Pilyi1+γ Pilyil+qPilyio+βPioyil+Y</td></tr><tr><td>DSC FL</td><td>2(1-pii)PilYii+Y 1 (1-pi1)Pil+yil+Y -αi∑j∈{0,1}(1-pij)7 log Pij</td></tr></table>
...
$$
\mathrm { D S C } ( x _ { i } ) = \frac { 2 p _ { i 1 } y _ { i 1 } + \gamma } { p _ { i 1 } + y _ { i 1 } + \gamma }
$$
...
$$
\mathrm { D L } = \frac { 1 } { N } \sum _ { i } \left[ 1 - \frac { 2 p _ { i 1 } y _ { i 1 } + \gamma } { p _ { i 1 } ^ { 2 } + y _ { i 1 } ^ { 2 } + \gamma } \right]
$$

Another version of $\mathrm { D L }$ is to directly compute setlevel dice coefficient instead of the sum of individual dice coefficient, which is easier for optimization:

$$
\mathrm { D L } = 1 - \frac { 2 \sum _ { i } p _ { i 1 } y _ { i 1 } + \gamma } { \sum _ { i } p _ { i 1 } ^ { 2 } + \sum _ { i } y _ { i 1 } ^ { 2 } + \gamma }
$$
...
$$
\mathrm { T I } = \frac { | A \cap B | } { | A \cap B | + \alpha | A \backslash B | + \beta | B \backslash A | }
$$
...
$$
\mathrm { T L } = \frac { 1 } { N } \sum _ { i } \left[ 1 - \frac { p _ { i 1 } y _ { i 1 } + \gamma } { p _ { i 1 } y _ { i 1 } + \alpha p _ { i 1 } y _ { i 0 } + \beta p _ { i 0 } y _ { i 1 } + \gamma } \right]
$$
...
![](images/c72ede0c96a57d478c5143d1e1bf277781e4eba424e632ac51a0230a979d1bba.jpg)  
...
$$
\mathrm { D S C } ( x _ { i } ) = \frac { 2 ( 1 - p _ { i 1 } ) p _ { i 1 } \cdot y _ { i 1 } + \gamma } { ( 1 - p _ { i 1 } ) p _ { i 1 } + y _ { i 1 } + \gamma }
$$
...
Results Table 7 shows the results. We find that replacing the training objective with DSC introduces performance boost for both settings, $+ 0 . 5 8$ for MRPC and $+ 0 . 7 3$ for QQP.

# 5 Ablation Studies

# 5.1 Datasets imbalanced to different extents

It is interesting to see how differently the proposed objectives affect datasets imbalanced to different extents. We use the paraphrase identification dataset QQP ( $3 7 \%$ positive and $63 \%$ negative) for studies. To construct datasets with different imbalance degrees, we used the original QQP dataset to construct synthetic training sets with different positive-negative ratios. Models are trained on these different synthetic sets and then test on the same original test set.

• Original training set (original) The original dataset with 363,871 examples, with $37 \%$ being positive and $63 \%$ being negative

# • Positive augmentation ( $\dagger$ positive)
...
Toma´s Ko ˇ cisk ˇ y, Jonathan Schwarz, Phil Blunsom, \` Chris Dyer, Karl Moritz Hermann, Gaabor Melis, and´ Edward Grefenstette. 2018. The narrativeqa reading comprehension challenge. Transactions of the Association of Computational Linguistics, 6:317–328.
